export default function ModelHero({ data }) {
  ...
  return (
    <section style={{ paddingBottom: 8 }}>
      {data.tagPill && <p dangerouslySetInnerHTML={{ __html: data.tagPill }} />}
      ...
      {data.primaryCta && (
        <p>
          <a href={data.primaryCta.href} dangerouslySetInnerHTML={{ __html: data.primaryCta.label }} />
        </p>
      )}

      {data.dataIntegrityNote && <p dangerouslySetInnerHTML={{ __html: data.dataIntegrityNote }} />}
      <hr />
    </section>
  );
}
