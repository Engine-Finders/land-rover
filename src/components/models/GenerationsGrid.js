    ...
    <section style={{ paddingBottom: 8 }}>
      ...
      {data.cards?.map((card) => (
        <div key={card.title}>
          ...
          {card.badge && <p dangerouslySetInnerHTML={{ __html: card.badge }} />}
          {card.meta && <p dangerouslySetInnerHTML={{ __html: card.meta }} />}
          {card.rating && <p dangerouslySetInnerHTML={{ __html: card.rating }} />}
          {card.verdict && (
            <p>
              <strong>Our Verdict:</strong>{" "}
              <span dangerouslySetInnerHTML={{ __html: card.verdict }} />
            </p>
          )}
          {card.cta && (
            <p>
